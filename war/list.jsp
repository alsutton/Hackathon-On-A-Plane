...
<%@ page import="com.hackathononaplane.server.datamodel.Hack" %>
    ...
    <div class="topbar">
      <div class="fill">
        <div class="container">
          ...
          <ul class="nav">
            <li><a href="/">Home</a></li>
            <li class="active"><a href="/list">See Hacks</a></li>
<%
...
<%
	if( request.getUserPrincipal() == null ) {
%>
<a href='<%=userService.createLoginURL("/list")%>'>sign in</a>
<%
	} else {
%>
${pageContext.request.userPrincipal.name} | <a href='<%=userService.createLogoutURL("/list")%>'>log out</a>
<%
	}
%>
		  </div>
    ...
    <div class="container">
      ...
      <div class="content">
        <div class="page-header">
          <h1>Other peoples hacks</h1>
        </div>
        <div class="row">
          <div class="span10">
			<p>Here's a list of hacks other people have made while in the air;</p>
			
			<div style="padding-top: 10px">&nbsp;</div>
<%
			int i = 0;
			for(Hack hack : ((Iterable<Hack>)request.getAttribute("hacks"))) {
				String name = hack.getUser().getNickname();
				if(name.indexOf('@') != -1) {
					name = name.substring(0,name.indexOf('@'));
				}
%>
			<div class="span9 odd<%=i%2%>">
				<div class="span5"><a href="<%=hack.getChangesetURL()%>"><%=hack.getName()%></a></div>
				<div class="span3"><%=name%> in<br/><%=hack.getPlaneTime()%> Air Minutes</div>
			</div>
<%
			}
%>			
